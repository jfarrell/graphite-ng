...
func init() {
	Functions["absolute"] = []string{"ProcessAbsolute", "metric"}
}

func ProcessAbsolute(dep_el chains.ChainEl) (our_el chains.ChainEl) {
	...
	go func(our_el chains.ChainEl, dep_el chains.ChainEl) {
		...
		for {
			d := <-dep_el.Link
			if d.Known {
				our_el.Link <- *metrics.NewDatapoint(d.Ts, math.Abs(d.Value), true)
			} else {
